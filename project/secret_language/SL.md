# random -> Inbuilt module : used for generating random characters.

# string -> Inbuilt module : Provides ready-made character sets like: lowercase letters, uppercase letters, digits.

    string.ascii_letters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    string.digits = "0123456789"

1. -> str : It is type hint that indicates that function returns a string.
   {symbol: ->, known as return type hint/return type annotation.}

2. ''.join(...) = It is a string method which combines everything in one string, without leaving any space.
   for example : ['z', 'o', 'x'] -> 'zox'

3. random.choices(..., k=n) = It is a function that picks random n number of items from given list/string.

4. 
